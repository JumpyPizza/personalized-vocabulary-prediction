...
class UserSimulationOracle(UserTruthOracle):
    ...
    def query_initial_samples(self): 

        # support_words : (words, labels)
        support_input, _, support_words = self.data_loader.get_data(support_batch_size = self.initial_sample_num, query_batch_size = 0)
...
class LLMActiveLearning:
    ...
    def __init__(self,
        ...
        word_freqs = [(word, word_frequency(word, "en")) for word in vocab_list]

        # Sort by frequency (ascending)
        word_freqs.sort(key=lambda x: x[1])  # lowest frequency first
        self.word_lowest_freq = word_freqs
    ...
    def run_active_learning_simulation(self): 
        ...
        for i in range(2):
            ...
            candidate_samples = self.word_lowest_freq[:10]
